...
const AllLists = (props) => {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(
        `${import.meta.env.VITE_BACKEND_URL}/shoppingList/${id}`
      );
      props.setShoppingLists(
        props.shoppingLists.filter((list) => list._id !== id)
      );
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <>
      ...
      <button onClick={() => props.handleAddListButtonClick()}>
        Add new List
      </button>
      {props.shoppingLists.map((list) => {
        return (
          <div key={list._id}>
            ...
            <button onClick={() => props.handleSelectedList(list)}>
              View Details
            </button>
